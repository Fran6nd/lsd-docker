...
-- ------------------------------------------------------- unruled building
--
-- The rules a client places blocks by are all enforced in one place:
-- LSd's default on_any_packet (lsd/src/funcs_packetrecv.c), which decides
-- a packet is sane or crap. on_sane_packet then dispatches it to
-- on_block_action / on_block_line and re-checks nothing. Both halves are
-- hookable, so returning "sane" here waves a packet through that the
...
-- line, at any range, in mid-air, with an empty inventory. Nothing here
-- draws anything; it only stops the server saying no.
--
...
--
-- NOT waived, and never to be: the three checks below are memory safety,
-- not rules. on_sane_packet hands the coordinates straight to
...
-- engine floor and the client's Root layer -- the same limit the block
-- api above keeps clear of.
--
-- Only while edit mode is on, and only for a builder. Edit mode admits
-- joiners now, so "anyone, always" would hand the map to whoever
-- wandered in.

getcfg("world_editor_freebuild", true);   -- let builders bypass the rules
...
function mod.on_any_packet(pid, data)
	if (editing and world_editor_freebuild and #data >= 1
	    and has_cap(pid, CAP_BUILD) and safe_block_packet(pid, data)) then
