...
--   unrestricted_blocks.lua
--               -- stops the server refusing block packets that break
--                  the placement rules, so a client can bring its own
--                  editor and express selections, copies and pastes as
--                  plain block packets. Gated on worldedit_build, which
--                  only exists while edit mode is on.
--
...
-- Stock scripts edit mode switches on, each with the cap that unlocks
-- it -- or no cap, when the script is gated on one we already grant.
local BORROWED  = {
	{script="sel",    cap="sel"},
	{script="noclip", cap="noclip"},
	{script="unrestricted_blocks"},   -- gated on CAP_BUILD, granted below
};
...
local function grant_all(pid)
	...
	for _, b in ipairs(BORROWED) do
		if (b.cap ~= nil and not has_cap(pid, b.cap)) then grant_cap(pid, b.cap); end
	end
end
...
local function edit_stop()
	for i in piditer(PID_BROADCAST) do
		if (is_joined(i)) then
			...
			for _, b in ipairs(BORROWED) do
				if (b.cap ~= nil and has_cap(i, b.cap)) then drop_cap(i, b.cap); end
			end
		end
	end
